...
async function persist(
  ...
  const scp =
    ...
    `-o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/root/.ssh/known_hosts -o BatchMode=yes ` +
    `/backup/dump.pgc ${env.sshUser}@${env.host}:${temp}`
  ...
  if (pruned.length > 0) {
    try {
      await sshContainer(env, key)
        .withExec(["sh", "-c", remoteScript(env, `cd ${dir} && rm -f -- ${pruned.join(" ")}`)])
        .sync()
    } catch (err) {
      throw infraError(
