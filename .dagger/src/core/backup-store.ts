...
export function tableCountScript(db: { dbContainer?: string; dbUser: string; database: string }): string {
  return (
    `docker exec -i ${db.dbContainer} psql -v ON_ERROR_STOP=1 -U ${db.dbUser} -d ${db.database} -tA ` +
    `<<'SHIPKIT_SQL'\n${TABLE_COUNT}\nSHIPKIT_SQL\n`
  )
}
...
export function storeScript(o: {
  ...
  return [
    "set -e",
    `local_sum=$(sha256sum ${o.local} | cut -d' ' -f1)`,
    o.remote(`umask 077 && mkdir -p ${o.dir} && test -w ${o.dir}`),
    o.upload,
    `remote_sum=$(${o.remote(`sha256sum ${o.temp}`)} | cut -d' ' -f1)`,
    ...
    o.remote(`chmod 600 ${o.temp} && mv -f ${o.temp} ${o.path}`),
    `echo "$local_sum"`,
  ].join("\n")
}
